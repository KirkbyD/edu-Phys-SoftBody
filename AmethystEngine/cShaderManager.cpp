...
bool cShaderManager::createProgramFromFile(std::string friendlyName, cShader& vertexShad, cShader& fragShader) {
	...
	if (!this->m_compileShaderFromSource(vertexShad, errorText)) {
		...
		glDeleteShader(vertexShad.ID);
		vertexShad.~cShader();

		return false;
	}//if ( this->vertexShad.ID(...
	...
	if (this->m_wasThereALinkError(curProgram.ID, errorText)) {
		...
		glDeleteProgram(curProgram.ID);
		curProgram.~cShaderProgram();
	...
	glDetachShader(curProgram.ID, fragShader.ID);
	glDetachShader(curProgram.ID, vertexShad.ID);
...
void cShaderManager::DeconstructShaderPrograms() {
	for (std::pair<unsigned, cShaderProgram> mp : m_ID_to_Shader)
		glDeleteProgram(mp.second.ID);
}
