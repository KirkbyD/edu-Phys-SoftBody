...
void cAIManager::update(sData& data) {
	...
	case nAI::eFormationBehaviours::CIRCLE:
	{
		...
		if (data.getSourceGameObj()->HasActor()) {
			if (data.getSourceGameObj()->GetActor()->_formation_pos > 1) {
				...
				data.getSourceGameObj()->GetActor()->setActiveBehaviour(nAI::eAIBehviours::SEEK);
			}
			//if its the anchor unit then don't override its movement as it will act as the conductor for the formation
		}
		break;
	}
	...
	case nAI::eFormationBehaviours::COLUMN:
	{
		...
		if (data.getSourceGameObj()->HasActor()) {
			if (data.getSourceGameObj()->GetActor()->_formation_pos > 1) {
				// set the target position based off of the desired unit position
				ai_behaviours.formColumn(data, fa);
				data.getSourceGameObj()->GetActor()->setActiveBehaviour(nAI::eAIBehviours::SEEK);
			}
			//if its the anchor unit then don't override its movement as it will act as the conductor for the formation  
		}
