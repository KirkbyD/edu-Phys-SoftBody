...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
	...
	if (key == GLFW_KEY_3 && action == GLFW_PRESS && !isDevCon) {
		...
		pPhysicsManager->WorldToggleWind();
	}
	...
	if (key == GLFW_KEY_6 && action == GLFW_PRESS && !isDevCon) {
		data.setSubsystem(ACTORS);
		data.setCMD(DEACTIVATE_FORMATIONS);
		pCmdMngr->IssueCommand(data);

		data.setCMD(SET_ACTIVE_AI);
		data.SetSteeringBehaviour(nAI::eAIBehviours::FLOCK);
		pCmdMngr->IssueCommand(data);
	}

	if (key == GLFW_KEY_7 && action == GLFW_PRESS && !isDevCon) {
		data.setSubsystem(ACTORS);
		data.setCMD(SET_ACTIVE_AI);
		data.SetSteeringBehaviour(nAI::eAIBehviours::IDLE);
		pCmdMngr->IssueCommand(data);

		data.setCMD(ACTIVATE_FORMATIONS);
		pCmdMngr->IssueCommand(data);
	}

	if (key == GLFW_KEY_8 && action == GLFW_PRESS && !isDevCon) {
		data.setSubsystem(ACTORS);
		data.setCMD(SET_ACTIVE_AI);
		data.SetSteeringBehaviour(nAI::eAIBehviours::FOLLOW_PATH);
		pCmdMngr->IssueCommand(data);
	}

	if (key == GLFW_KEY_9 && action == GLFW_PRESS && !isDevCon) {
		data.setSubsystem(AI);
		data.setCMD(INVERT_PATH);
		pCmdMngr->IssueCommand(data);
	}

	if (key == GLFW_KEY_0 && action == GLFW_PRESS && !isDevCon) {
		data.setSubsystem(ACTORS);
		data.setCMD(SET_ACTIVE_AI);
		data.SetSteeringBehaviour(nAI::eAIBehviours::IDLE);
		pCmdMngr->IssueCommand(data);
	...
	if (key == GLFW_KEY_MINUS && action == GLFW_PRESS && !isDevCon) {
		data.setSubsystem(ACTORS);
		data.setCMD(DEACTIVATE_FORMATIONS);
		pCmdMngr->IssueCommand(data);

		data.setCMD(SET_ACTIVE_AI);
		data.SetSteeringBehaviour(nAI::eAIBehviours::FLOCK_PATH);
		pCmdMngr->IssueCommand(data);
	}

	if (key == GLFW_KEY_EQUAL && action == GLFW_PRESS && !isDevCon) {
		data.setSubsystem(ACTORS);
		data.setCMD(SET_ACTIVE_AI);
		data.SetSteeringBehaviour(nAI::eAIBehviours::IDLE);
		pCmdMngr->IssueCommand(data);
	...
	if (key == GLFW_KEY_KP_1 && action == GLFW_PRESS && !isDevCon) {
		data.setSubsystem(ACTORS);
		data.setCMD(SET_SEPERATION_WEIGHT);
		data.increaseFlockWeight(0.1f);
		pCmdMngr->IssueCommand(data);
	}

	if (key == GLFW_KEY_KP_2 && action == GLFW_PRESS && !isDevCon) {
		data.setSubsystem(ACTORS);
		data.setCMD(SET_ALIGNMENT_WEIGHT);
		data.increaseFlockWeight(0.1f);
		pCmdMngr->IssueCommand(data);
	}

	if (key == GLFW_KEY_KP_3 && action == GLFW_PRESS && !isDevCon) {
		data.setSubsystem(ACTORS);
		data.setCMD(SET_COHESION_WEIGHT);
		data.increaseFlockWeight(0.1f);
		pCmdMngr->IssueCommand(data);
	}

	if (key == GLFW_KEY_KP_4 && action == GLFW_PRESS && !isDevCon) {
		data.setSubsystem(ACTORS);
		data.setCMD(SET_SEPERATION_WEIGHT);
		data.decreaseFlockWeight(0.1f);
		pCmdMngr->IssueCommand(data);
	}

	if (key == GLFW_KEY_KP_5 && action == GLFW_PRESS && !isDevCon) {
		data.setSubsystem(ACTORS);
		data.setCMD(SET_ALIGNMENT_WEIGHT);
		data.decreaseFlockWeight(0.1f);
		pCmdMngr->IssueCommand(data);
	...
	if (key == GLFW_KEY_KP_6 && action == GLFW_PRESS && !isDevCon) {
		data.setSubsystem(ACTORS);
		data.setCMD(SET_COHESION_WEIGHT);
		data.decreaseFlockWeight(0.1f);
		pCmdMngr->IssueCommand(data);
